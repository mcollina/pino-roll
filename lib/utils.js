...
const { format, parse } = require('temporal-fmt')
const { promisify } = require('util')
// https://github.com/fullcalendar/temporal-polyfill/blob/main/polyfill/README.md#package-entrypoints
// ensures polyfill is used if native impl is not available
...
function parseFrequency (frequency) {
  ...
  if (frequency === 'weekly') {
    ...
    return { frequency, start, next: getNextWeek() }
  }
 ...
 */
function nextWeeklyOccurrence (now, weekday, eventTime = new Temporal.PlainTime(0, 0, 0, 0), eventTimeZone = Temporal.Now.timeZoneId()) {
 ...
 * @param {string} tz - Time zone of date
 * @returns {number} - Unix Epoch
 */
function getNextDay (start, tz = Temporal.Now.timeZoneId()) {
  const instant = Temporal.Instant.fromEpochMilliseconds(start).toZonedDateTimeISO(tz).with({ hour: 0 }).add({ days: 1 })
 ...
 * @returns {number} - Unix Epoch
 */
function getNextHour (start, tz = Temporal.Now.timeZoneId()) {
  const instant = Temporal.Instant.fromEpochMilliseconds(start).toZonedDateTimeISO(tz).with({ minute: 0 }).add({ hours: 1 })
  return instant.epochMilliseconds
}
 ...
 * @returns {number} Unix Epoch
 */
function getNextWeek (nowTemp = Temporal.Now.zonedDateTimeISO(), tz = Temporal.Now.timeZoneId()) {
  const n = nextWeeklyOccurrence(nowTemp, 1, undefined, tz)

  return n.epochMilliseconds
}

function getNextCustom (frequency, time = Date.now()) {
  return time - time % frequency + frequency
}

function getNext (frequency, tz = Temporal.Now.timeZoneId(), date = Temporal.Now.instant().epochMilliseconds) {
  if (frequency === 'daily') {
    return getNextDay(Temporal.Instant.fromEpochMilliseconds(date).toZonedDateTimeISO(tz).with({ minute: 0, hour: 0, second: 0, millisecond: 0 }).epochMilliseconds, tz)
  }
  if (frequency === 'hourly') {
    return getNextHour(Temporal.Instant.fromEpochMilliseconds(date).toZonedDateTimeISO(tz).with({ minute: 0, second: 0, millisecond: 0 }).epochMilliseconds, tz)
  }
  if (frequency === 'weekly') {
    return getNextWeek(Temporal.Instant.fromEpochMilliseconds(date).toZonedDateTimeISO(tz), tz)
  }
  return getNextCustom(frequency, date)
}
...
function identifyLogFile (checkedFileName, fileVal, dateFormat, extension) {
  ...
  if (typeof dateFormat === 'string' && dateFormat.length > 0) {
    try {
      const d = parse(dateFormat, checkFileNameSegments[0])
      if (d instanceof Temporal.PlainDate) {
        fileTime = d.toPlainDateTime().epochMilliseconds
      } else {
        fileTime = d.epochMilliseconds
      }
    } catch (e) {
      return false
    }
...
async function removeOldFiles ({ count, removeOtherLogFiles, baseFile, dateFormat, extension, createdFileNames, newFileName }) {
  if (!removeOtherLogFiles) {
    ...
    if (files.length > count) {
      ...
      await Promise.allSettled(filesToRemove.map((file) => unlinkWithRetry(join(...pathSegments, file.fileName))))
    }
  }
}
...
function parseDate (formatStr, frequencySpec, parseStart = false) {
  ...
  try {
    const d = Temporal.Instant.fromEpochMilliseconds(parseStart ? frequencySpec.start : frequencySpec.next).toZonedDateTimeISO(Temporal.Now.timeZoneId())
