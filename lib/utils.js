...
async function removeOldFiles ({ count, removeOtherLogFiles, baseFile, dateFormat, extension, createdFileNames, newFileName }) {
  console.log(`[removeOldFiles] Called with count=${count}, removeOtherLogFiles=${removeOtherLogFiles}, newFileName=${newFileName}`)
  if (!removeOtherLogFiles) {
    ...
    if (createdFileNames.length > count) {
      ...
      console.log(`[removeOldFiles] Removing ${filesToRemove.length} files:`, filesToRemove)
    ...
    console.log(`[removeOldFiles] Found ${files.length} matching log files`)
    ...
    console.log(`[removeOldFiles] files.length=${files.length}, count=${count}, will remove ${filesToRemove.length} files:`, filesToRemove.map(f => f.fileName))
