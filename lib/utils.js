...
function getNextWeek () {
  ...
  const monday = new Date(today)
  monday.setDate(today.getDate() - ((today.getDay() + 6) % 7))
  return addDays(monday, 7).setHours(0, 0, 0, 0)
}
...
function createSymlinkSync (fileVal) {
  ...
  try {
    ...
    // The link only exists so the active file can be tailed under a fixed name. Failing to update
    // it must not reach the log stream as an error and take the process down.
    process.emitWarning(`could not update ${linkPath}: ${error.message}`)
  }
  return false
}
