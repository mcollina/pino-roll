...
function parseFrequency (frequency) {
  ...
  if (frequency === 'daily') {
    ...
    const next = new Date(start + 24 * 60 * 60 * 1000).setHours(0, 0, 0, 0)
    return { frequency, start, next }
  } else if (frequency === 'hourly') {
    const start = today.setMinutes(0, 0, 0)
    const next = new Date(start + 60 * 60 * 1000).setMinutes(0, 0, 0)
    return { frequency, start, next }
  } else if (typeof frequency === 'number') {
    return { frequency, next: today.getTime() + frequency }
  }
  return null
}
...
module.exports = { buildFileName, detectLastNumber, parseFrequency, parseSize }
